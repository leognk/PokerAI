...
std::vector<std::vector<TreeTraverser::seq_t>> TreeTraverser::traverseTree()
{
	...
	for (uint8_t r = 0; r < egn::N_ROUNDS; ++r) {
		...
		if (verbose)
			std::cout
				<< opt::toUpper(egn::roundToString(r)) << "\n"
				<< "states: " << opt::prettyNum(seqsToCurrentRound.size(), 1) << "\n";
	...
	opt::time_t startTime)
{
	...
	std::cout
		<< "nodes: " << std::setw(6) << opt::prettyNum(nNodes, 1)
		<< " | seq: " << std::setw(6) << opt::prettyNum(nActionSeq, 1)
		<< " | finishedSeq: " << std::setw(6) << opt::prettyNum(nFinishedSeq, 1)
		<< " | continuingSeq: " << std::setw(6) << opt::prettyNum(nContinuingSeq, 1)
