...
void printActions(const std::vector<uint8_t>& actions, unsigned& count)
{
	abcInfo_t infoset(
		bp::MAX_PLAYERS,
		bp::ANTE,
...
void BlueprintCalculator::traverseMCCFR(uint8_t traverser)
{
	...
	while (true) {
		...
		if (abcInfo.state.finished || !abcInfo.state.isAlive(traverser)) {
			...
			lastChild.push_back(a == 0);
			////////////////////////////////////////////////////////////////////////////////////
			//bool y = abcInfo.state.stakes[0] == 9600 && abcInfo.state.stakes[1] == 9900 && abcInfo.state.stakes[2] == 9900 && abcInfo.state.round == egn::FLOP && a == 2;
			//int yolo = 0;
			////////////////////////////////////////////////////////////////////////////////////
		}
