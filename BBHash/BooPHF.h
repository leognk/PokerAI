...
#pragma warning(suppress: 6308)
		...
		{
			uint64_t oldval = InterlockedOr(_bitArray + (pos >> 6), (uint64_t) (1ULL << (pos & 63)) );
		...
		void set(uint64_t pos)
		{
			...
			InterlockedOr(_bitArray + (pos >> 6ULL), (1ULL << (pos & 63)) );
		}
		...
		{
			//_bitArray [pos >> 6] &=   ~(1ULL << (pos & 63) ) ;
			InterlockedAnd(_bitArray + (pos >> 6ULL), ~(1ULL << (pos & 63) ));
		}
...
//#define NBBUFF 2
	...
	public:
		...
		{
			uint64_t nb_done =0;
			unsigned tid =  InterlockedIncrement(&_nb_living);
			auto until = *until_p;
			uint64_t inbuff =0;
...
#pragma warning(suppress: 4244)
			_pid = _getpid() + printPt(pthread_self());
